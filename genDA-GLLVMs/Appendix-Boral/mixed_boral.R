...
results.boral.mixed <- foreach(i = 1:TRIALS,
                      .combine = rbind,
                      ...
                      .options.snow = opts) %dopar%{
                        ...
                        for(s in 1:length(m_vals)){
                          ...
                          for (j in 1:m) {
                            ...
                            t1 <- system.time({
                              res_boral <- boral(y=mY, X= mX,family=f,lv.control = list(num.lv = 2), save.model = TRUE)
                            })
 ...
 b5a <- mean(vphi[which(f=="normal")] - res_boral$lv.coefs.mean[which(f=="normal"), 4] )
 
 # RMSE for vphi
 
 r5a <- sqrt(mean((vphi[which(f=="normal")] - res_boral$lv.coefs.mean[which(f=="normal"), 4] )^2))
 
 # CI_width for vphi
 
 u_phi_boral = res_boral$lv.coefs.mean[which(f=="normal")] +res_boral$lv.coefs.sd[which(f=="normal"), 4]*q
 l_phi_boral = res_boral$lv.coefs.mean[which(f=="normal")] -res_boral$lv.coefs.sd[which(f=="normal"), 4]*q
 
 
 CI_5a <- mean(abs((u_phi_boral- l_phi_boral)))
 
 # Coverage for vphi
 
 CO_5a <- mean( vphi[which(f=="normal")] >= l_phi_boral & vphi[which(f=="normal")] <= u_phi_boral)
 
                            
                            
                   # ----------------------------------- #   
                            
 vals_boral  <- c(p1a, p2a, b3a, r3a, CI_3a, CO_3a, b4a, r4a, CI_4a, CO_4a, b5a, r5a, CI_5a, CO_5a, round(t1[3], 4))
...
save(results.boral.mixed, file="Mixed_boral.RData")
